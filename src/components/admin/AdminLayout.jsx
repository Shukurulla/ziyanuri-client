...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-[260px] bg-[#0f1117] transform transition-transform lg:translate-x-0 lg:static ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="px-5 py-5 flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-accent-500 to-accent-600 rounded-xl flex items-center justify-center font-bold text-white text-sm shadow-lg shadow-accent-500/30">
            {" "}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-white/5">
          ...
          <button
            onClick={logout}
            className="flex items-center gap-2 text-white/40 hover:text-red-400 text-xs w-full px-3 py-2 rounded-lg hover:bg-white/5 transition-all"
          >
